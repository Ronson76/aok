...
export default function AdminUsersReport() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(user => 
        ...
        (user.createdAt && format(new Date(user.createdAt), "dd/MM/yyyy").includes(query))
      );
    }
  ...
  const filterByDateRange = (items: any[]): any[] => {
  ...
  const downloadPDF = () => {
    ...
    const dataToExport = filterByDateRange(filteredUsers);
    ...
    const tableData = dataToExport.map(user => [
      user.orgClientReferenceCode || user.name || "-",
      user.email || "-",
      user.accountType || "-",
    ...
    (doc as any).autoTable({
      startY: 45,
      head: [["Name/Reference", "Email", "Account Type", "Registered"]],
      body: tableData,
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground">Complete list of registered users</p>
              </div>
            </div>
            <Button onClick={() => setShowPdfDialog(true)} disabled={!users || users.length === 0} data-testid="button-download-users-pdf">
                  ...
                  >
                    <div>
                      <p className="font-medium">
                        {user.orgClientReferenceCode || user.name || "Unnamed"}
                      </p>
