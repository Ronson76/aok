...
- **Admin Dashboard**: Role-based access, user/organization management, license agreements, revenue tracking, feature permissions, security audit logging. Enterprise feature toggles (Organisation Dashboard, Assurance Dashboard, API Access) with optional expiry dates per organisation.
...
- **Enterprise Add-ons**: Organisation Dashboard (main org management view), Assurance Dashboard (real-time safeguarding position, risk heatmaps, board governance reports) and API Access (read-only external integration) — toggled on/off per organisation by admin with optional expiry dates.
